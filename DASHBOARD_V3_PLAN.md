...
- 2026-07-16: (นอกแผน — ผู้ใช้ขอ) ✅ /divmanage เพิ่ม modal checklist per-coin ให้เหมือน dashboard (งานค้างจากตอน mirror strip 886648a): ปุ่ม 📋 ต่อแถวหน้า strip → modal 6 ขั้นข้อความเดียวกับ #modal-checklist เป๊ะ — ข้อ auto เติมสถานะ+ค่าที่วัดจาก `ck` ของ /coinstatus (แก้ได้ ไม่ล็อก) / ข้อ manual (SMC, s4plan, ขั้น 6) ติ๊กเอง / progress+verdict+ล้าง (ล้างเฉพาะ manual — auto เติมกลับ) / ปิดแล้วเปิดเหรียญเดิม = tick ค้างไว้, สลับเหรียญ = ล้าง, ไม่มีข้อมูล = generic mode; tip ช่อง 6 เปลี่ยนเป็น "ติ๊กเองใน 📋"; verify: tsc ผ่าน + Playwright 390px บน HTML ที่ render จริงจาก divManagePage + mock /coinstatus — 26 เช็คผ่านหมด (auto-fill สี/ค่า, verdict flip ที่ 6/6, reset, reopen, switch coin, generic, chip→msg regression) ไม่มี pageerror — deploy จริงรอ merge เข้า main (deploy-worker.yml ยิงเอง)
